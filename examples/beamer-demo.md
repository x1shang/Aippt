...
> 本文件是 AIPPT 的默认示例（界面「载入示例 Markdown」载入的就是它）。
> 用「仅排版」模式即可一键复现全部效果；同目录下的 `references.bib` 会被自动载入，用于本文的文献引用。
> 你也可以直接用记事本改这个文件（就在安装目录的 examples/ 下）。
...
由 \eqref{eq:pyth} 可得定理 \ref{thm:pyth} 的结论；相关论述可参考 \cite{knuth1998} 与 [@li2021]。

> 引用写法支持 `\cite{key}`、`\citep{}`、`\citet{}`、`[@key]`；编号按首次出现顺序分配，
> 文末会**自动追加「参考文献」页**。文献库可以用界面导入 `.bib`，也可以直接写在 md 的开头注释里。
...
- 行内标记：**加粗**、*斜体*、`等宽`、==高亮==、[[示例双链|Obsidian 别名]]
  - 二级要点缩进 2 空格
    - 三级要点缩进 4 空格
1. 有序列表也一样支持
2. 第二条
...
把图片放在 md 同目录，三种写法都行：`![](figure.png)`、`![[figure.png]]`、`\includegraphics[width=0.6\textwidth]{figure}`；
会按比例缩放并自动加题注（本示例不放图片，避免污染你的目录）。
